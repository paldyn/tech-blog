...
```java
// Java 10부터 사용 가능 (local variable type inference)
var message = "Hello";   // 컴파일러가 String으로 추론
var count = 42;          // int로 추론
```

### 자동 메모리 관리(Garbage Collection)
C/C++에서는 개발자가 직접 `malloc`/`free`로 메모리를 관리해야 합니다. Java는 **GC(Garbage Collector)** 가 사용하지 않는 객체를 자동으로 회수합니다. 메모리 누수와 댕글링 포인터 버그를 원천적으로 방지합니다. 다만 GC 동작을 이해하지 못하면 성능 문제를 디버깅하기 어렵기 때문에 별도의 글에서 깊게 다룹니다.
...
이 블로그는 **Java SE**를 중심으로 다루고, Jakarta EE 관련 내용은 필요시 언급합니다. 에디션별 차이는 'SE · EE · ME · Jakarta EE' 글에서 자세히 설명합니다.
...
```java
// Spring Boot 3.x + Java 21 LTS 진입점 예시
...
Java의 역사부터 최신 LTS(Java 21)까지, 언어 문법부터 JVM 내부, GC 튜닝, 모듈 시스템, 테스트, 보안, 마이그레이션까지 종합적으로 다룹니다.
