...
[지난 글](/posts/orm-migration-flyway-liquibase-alembic/)에서 DB 마이그레이션 도구를 살펴봤습니다. 이번 글에서 다룰 실전 SQL 패턴은 **페이지네이션**입니다. 대부분의 API에는 목록 조회 엔드포인트가 있고, 거의 모든 목록 조회에는 페이지네이션이 필요합니다. 어떤 방식을 선택하느냐에 따라 성능과 데이터 정확성이 크게 달라집니다.
