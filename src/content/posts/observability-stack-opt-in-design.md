...
- self-hosted (on-prem) 형태로 배포되는 한 백엔드 시스템. 5개 서비스
  (Spring Boot 4개 + nginx 1개)
...
services:
  ...
  promtail:
    ...
    # docker_sd_configs filter: label "app.observe=true"
  ...
  service-auth:
    labels:
      app.observe: "true"      # ← Promtail 이 이걸 보고 수집
```
...
비즈 메트릭은 직접 추가 가능 — 우리는 PG↔mirror 미러 drift 같은 건 별도
gauge 등록:

```java
meterRegistry.gauge("app_data_mirror_drift",
...
services:
  service-auth:
    ...
    environment:
      ...
      OTEL_SERVICE_NAME: service-auth
...
기본 2종 번들 포함:
- **App Overview** — service up/down, HTTP rate / error / p95 latency,
  JVM heap, HikariCP, 최근 ERROR 로그
- **App Operations** — 로그인 / 429 차단 / 5xx / 401·403, PG↔mirror
...
| 지표 | 도입 전 | 도입 후 (활성 시) |
|---|---|---|
| 로그 검색 | `docker compose logs` grep | LogQL 쿼리 (`{service="service-auth"} \|~ "ERROR"`) |
| 메트릭 | 없음 | Prometheus 30일 보존, App Overview 대시보드 |
