...
정리하면 선택 기준은 명확합니다. **커밋 정보(작성자·메시지)를 보존하며 커밋까지 만들고 싶다면 `git am`**, <strong>변경 내용만 작업 트리에 얹고 커밋은 직접 구성하고 싶다면 `git apply`</strong>입니다. 코드 리뷰 중 받은 diff를 잠깐 적용해 확인하거나, CI 스크립트에서 패치 적용 가능성만 검사하는 상황이라면 `git apply`가 훨씬 가볍고 적합합니다.
