...
[지난 글](/posts/java-temporal-adjusters/)까지 java.time의 주요 도구를 모두 살펴봤습니다. 날짜·시간 챕터의 마지막인 이번 글은 다시 과거로 갑니다. `java.time`이 나온 지 10년이 넘었지만, 오래된 코드베이스와 레거시 라이브러리 인터페이스에는 여전히 `Date`와 `Calendar`가 살아 있습니다. 이 글은 그 코드를 **유지보수하거나 마이그레이션해야 하는 사람**을 위해, 레거시 API의 함정을 구체적으로 정리하고 안전한 전환 전략을 다룹니다.
