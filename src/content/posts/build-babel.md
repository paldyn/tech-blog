...
[지난 글](/posts/ts-incremental-adoption/)에서 TypeScript를 기존 JavaScript 프로젝트에 점진적으로 도입하는 전략을 살펴봤습니다. 이번에는 **빌드 도구**를 다룹니다. 첫 주제는 Babel입니다. "Babel은 최신 JS를 구형 브라우저에서 돌아가도록 변환해준다"는 것은 누구나 알지만, 내부에서 정확히 무슨 일이 일어나는지 아는 개발자는 많지 않습니다. 파이프라인을 해부하면 플러그인 작성, 성능 최적화, 트러블슈팅 모두가 쉬워집니다.
