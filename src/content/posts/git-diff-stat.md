...
[지난 글](/posts/git-diff-word-level/)에서 `--word-diff`로 변경을 단어 단위까지 세밀하게 들여다봤다. 이번엔 정반대 방향이다. 수십 개 파일이 바뀐 큰 변경 앞에서 diff를 전부 읽기 전에, "어떤 파일이 얼마나 바뀌었나"를 먼저 파악하고 싶을 때가 있다. <strong>`git diff --stat`</strong>은 변경 내용 대신 **변경 규모**를 파일별 막대로 요약해, 전체 그림을 한눈에 보여준다.
