...
[지난 글](/posts/http-cache-control/)에서 Cache-Control 지시어를 살펴봤다. 이번 글에서는 캐시가 <strong>"지금 이 응답이 아직 유효한가"</strong>를 판단하는 신선도 알고리즘과, 만료된 캐시를 효율적으로 갱신하는 **재검증(Validation)** 메커니즘을 완전히 해설한다.
