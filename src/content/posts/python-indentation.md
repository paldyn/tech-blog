...
[지난 글](/posts/python-comments/)에서 주석 작성법을 살펴봤다. 이번 글에서는 Python을 처음 배우는 사람들이 가장 당황하는 특징 중 하나인 **들여쓰기**(indentation)를 다룬다. 대부분의 언어는 들여쓰기를 가독성을 위한 "권장 스타일"로 본다. 중괄호(`{}`)나 키워드(`begin`/`end`)로 블록을 표시하기 때문에 들여쓰기 없이도 코드가 실행된다. Python은 다르다. 들여쓰기가 **문법(syntax) 그 자체**다. 들여쓰기가 틀리면 코드가 실행되지 않는다.
