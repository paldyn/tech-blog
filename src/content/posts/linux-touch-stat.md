...
[지난 글](/posts/linux-mkdir-rmdir/)에서 디렉터리를 만들고 지우는 방법을 배웠다. 이번 글에서는 <strong>파일을 빈 상태로 생성하거나 타임스탬프를 조작하는 `touch`</strong>와 <strong>파일의 모든 메타데이터를 조회하는 `stat`</strong>을 다룬다. 두 명령어는 쉘 스크립트에서 조건 판단이나 빌드 시스템에서 자주 등장한다.
