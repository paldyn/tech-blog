...
지난 [트랜스포머(Transformer)란 무엇인가](/posts/transformer-basics/) 글에서 트랜스포머가 "문장을 어떻게 이해하는지"를 살펴봤다. 어텐션 메커니즘이 단어 간의 관계를 수치로 계산하고, 그 결과를 바탕으로 다음 토큰을 예측한다는 흐름이었다. 그런데 여기서 자연스러운 질문이 생긴다. **같은 LLM에게 질문해도 왜 어떤 때는 훌륭한 답이 나오고, 어떤 때는 엉뚱한 답이 나오는 걸까?**
