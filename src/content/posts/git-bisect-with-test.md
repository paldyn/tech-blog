...
[지난 글](/posts/github-secrets/)에서 CI가 다루는 민감 정보를 Secrets로 안전하게 관리하는 법을 다뤘다. CI 테스트가 어느 날 갑자기 빨간불이 됐다고 하자. 분명 지난주엔 멀쩡했는데, 그 사이 수십 개의 커밋이 쌓였다. 어느 커밋이 버그를 들여왔을까? 하나씩 되돌려가며 확인하는 건 시간 낭비다. <strong>`git bisect`</strong>는 이 문제를 이분 탐색으로 풀어, 정상이던 커밋과 망가진 커밋 사이를 절반씩 좁혀가며 **첫 실패 커밋**을 찾아낸다.
