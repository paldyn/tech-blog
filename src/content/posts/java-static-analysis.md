...
[지난 글](/posts/java-coding-conventions/)에서 컨벤션을 도구로 강제하는 자동화 계층을 다뤘다. 그 계층의 마지막이자 가장 강력한 조각이 정적 분석이다. 스타일 검사가 "이 코드가 규칙에 맞게 생겼는가"를 본다면, 정적 분석은 한 걸음 더 나아가 "이 코드가 실제로 버그를 품고 있는가"를 코드를 **실행하지 않고** 찾아낸다. 이번 글에서는 정적 분석이 무엇을 어떻게 잡아내는지, 왜 일찍 잡을수록 이득인지, 그리고 실전에서 오탐과 어떻게 공존하는지를 정리한다.
