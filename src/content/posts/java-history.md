...
[지난 글](/posts/java-what-is-java/)에서 Java가 언어이자 플랫폼임을 살펴봤습니다. 그런데 지금의 Java는 처음부터 이런 모습이 아니었습니다. Java는 **가전제품 소프트웨어 플랫폼**으로 시작해, 웹 애플릿 시대를 거쳐, 엔터프라이즈 서버의 왕좌에 올랐고, 이제는 클라우드·컨테이너 시대에 맞게 다시 진화 중입니다. 각 시대의 문제 의식이 버전별 기능에 그대로 녹아 있기 때문에, 역사를 알면 "왜 이 기능이 이 모양인가?"가 자연스럽게 이해됩니다.
