...
### Java 1.4 (2002) — 본격적인 엔터프라이즈

`java.nio`, 정규식(`java.util.regex`), `assert` 키워드, JAXP XML 파서 등이 추가됐습니다. 이 시기 Java는 엔터프라이즈 서버 시장을 장악했고, Struts, Hibernate, Spring 같은 프레임워크가 등장했습니다.
...
list.add("hello");
String s = list.get(0); // 캐스팅 불필요, 안전
```
...
Scripting API, JDBC 4.0, JAXB 2.0, JAX-WS 등 주로 엔터프라이즈 API 개선이 이뤄졌습니다.
...
```java
// Java 10 LTS부터 사용 가능 (단, Java 10은 LTS 아님)
var list = new ArrayList<String>();   // ArrayList<String>으로 추론
var path = Path.of("/tmp/data.txt");  // Path로 추론
```
...
Thread platformThread = new Thread(() -> System.out.println("플랫폼"));

// Java 21: Virtual Thread (경량 스레드, JVM 관리)
...
```text
Java 8 LTS  → (3년 갭) →  Java 9 → 10 → Java 11 LTS → 12 → ... → Java 17 LTS → 18 → 19 → 20 → Java 21 LTS
    2014                    2017    2018     2018       ...         2021          ...                 2023
```

- **LTS(Long-Term Support)**: 약 3년 주기로 출시, 최소 8년 유지보수 (Oracle 기준)
- **비LTS**: 6개월마다 출시, 다음 버전 나오면 EOL

실무에서는 LTS 버전만 사용하는 것이 일반적입니다. 현재 활성 LTS는 Java 8, 11, 17, 21이며, 다음 LTS는 Java 25(2025년 9월 예정)입니다.
