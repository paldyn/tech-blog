...
감사 로그는 평소에는 존재감이 없지만, 사고가 터진 순간 "그날 무슨 일이 있었는가"를 증언할 수 있는 유일한 기록이다. Metadata를 기본으로, 민감 리소스는 본문 금지, 파괴적 변경은 상세 기록 — 이 세 원칙으로 정책을 세우고 외부로 내보내 보존하자. 이것으로 관측성 이야기가 마무리됐다. 다음 글부터는 클러스터 상태를 Git 저장소와 자동으로 동기화하는 운영 패러다임, **GitOps**를 다룬다.
