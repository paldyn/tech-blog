...
- self-hosted 백엔드 시스템. 백엔드 4개 (인증 / 도메인 A / B / C
  서비스) + 프론트 1개 + 릴리스 오케스트레이터까지 6개 레포로 구성
...
`react: ^18.3.1` 와 `react-dom: ^19.2.5` 가 동시에 박혀 있는 모순.
lockfile 은 18.3.1 로 고정되어 있어 vite build 는 통과하지만, 정직한
npm install 한 번 돌리면 깨지는 상태.
...
- **trivy-action@0.24.0** — 처음 publish.yml 작성 시 잘못 핀한 버전.
  Dependabot 이 일부 레포엔 PR 안 만들어서 두 백엔드 API 에는 잘못된
  버전이 그대로 남아있음
- **SARIF 업로드** — private repo + GitHub Advanced Security 미활성 환경
  에서 권한 부족

![62개 PR 일괄 머지 → 빌드 폭탄 — 5개 source repo 모두 실패](/assets/posts/dependabot-pr-flood-impact.svg)
...
+FROM eclipse-temurin:21-jre-jammy   # 인증 서비스
+FROM eclipse-temurin:17-jre-jammy   # 도메인 A / B / C
```
...
![dependabot.yml ignore 룰 — 전후 비교](/assets/posts/dependabot-ignore-strategy.svg)
...
frontend 쪽도 동일하게 react / electron / vite / fortawesome 메이저 ignore.
...
|---|---|---|
| 이미지 빌드 성공 | 0 / 5 | 5 / 5 |
| 릴리스 워크플로우 | failure (15분) | success (8분 55초) |
...
=== source repos ===
  ...
  인증 서비스          : completed / success
  도메인 A          : completed / success
  도메인 B           : completed / success
  도메인 C   : completed / success
```
...
4. **운영에 영향 없는 이유 명시** — 빌드 실패는 새 이미지 미생성으로
   끝나서 기존 운영 환경 그대로 작동했다. 다행. 그래도 다음 릴리스
   시점까지 fix 는 필수.
