...
[지난 글](/posts/python-cache-decorator/)에서 캐시 데코레이터를 살펴봤다. 이번 글에서는 **deprecated 경고 데코레이터**를 다룬다. 라이브러리나 큰 코드베이스에서 오래된 API를 교체할 때, 기존 코드를 당장 깨지 않으면서 사용자에게 "이 함수는 곧 사라진다"는 경고를 보내는 패턴이다.
...
여기까지 Python 데코레이터의 본질부터 타입 힌트, 상태 관리, 캐싱, 경고까지 실무에서 필요한 패턴을 모두 살펴봤다. 다음 글에서는 컨텍스트 매니저를 다룬다.
