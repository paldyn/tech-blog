...
[지난 글](/posts/python-typing-self-class/)에서 타입 힌트 이야기를 마무리했다. 이번 글부터는 코드를 작성하는 일에서 한 걸음 물러나, 그 코드가 의존하는 외부 라이브러리를 어떻게 가져오고 관리하는지를 다룬다. 그 출발점이 `pip`다. 파이썬을 설치하면 함께 따라오는 표준 패키지 설치 도구로, `import requests` 한 줄을 가능하게 만드는 모든 일이 사실 pip에서 시작된다.
