...
Celery의 본질은 "오래 걸리는 일을 지금 처리하지 말고 큐에 맡긴다"는 한 문장이다. 요청은 빠르게 응답하고, 무거운 작업은 워커가 뒤에서 안정적으로 처리하며, 결과는 나중에 추적한다. 다음 글에서는 데이터베이스로 다시 돌아가, ORM과 SQL을 한 라이브러리에서 모두 다루는 강력한 도구 SQLAlchemy를 살펴본다.
