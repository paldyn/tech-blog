...
서버가 Range를 받아들이면 `200 OK`가 아니라 <strong>`206 Partial Content`</strong>로 응답하고, 어떤 구간을 전체 중 어디에서 잘라 보냈는지를 `Content-Range`로 알려 준다.
...
파일이 1000바이트뿐인데 `Range: bytes=5000-6000`을 요청하면 서버는 줄 수 있는 게 없다. 이때 <strong>`416 Range Not Satisfiable`</strong>로 응답하고, `Content-Range`에 전체 크기만 담아 "유효 범위는 여기까지"라고 알린다.
...
- **Accept-Ranges: bytes**로 서버가 부분 전송을 광고하고, 클라이언트는 <strong>`Range: bytes=`</strong>로 구간을 요청한다.
