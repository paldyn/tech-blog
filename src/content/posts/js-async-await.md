...
지난 [Promise — 비동기를 값으로 다루다](/posts/js-promise/) 글에서 Promise가 콜백 헬을 어떻게 해결하는지 살펴봤습니다. `.then()` 체이닝으로 비동기 흐름을 선형으로 표현할 수 있게 되었습니다. 그런데 복잡한 로직, 특히 조건 분기나 반복문이 섞이면 `.then()` 체인도 금세 읽기 어려워집니다.
...
**지난 글:** [Promise — 비동기를 값으로 다루다](/posts/js-promise/)

**다음 글:** [비동기 패턴 — 병렬, 순차, 재시도, 타임아웃](/posts/js-async-patterns/)
