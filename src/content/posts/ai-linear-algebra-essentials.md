...
[지난 글](/posts/ai-current-landscape/)에서 AI 생태계의 전체 지도를 살펴봤다. 이제부터는 수학 기초 파트가 시작된다. "AI 공부에 수학이 정말 필요한가?" 라고 묻는 사람이 많다. 필요하다. 단, 모든 수학이 필요한 게 아니라 **특정 영역**의 수학이 필요하다. 그 첫 번째가 선형대수(Linear Algebra)다. 선형대수 없이는 임베딩, 신경망, Attention 메커니즘 중 어느 하나도 제대로 이해할 수 없다.
