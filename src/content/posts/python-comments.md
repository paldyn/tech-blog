...
[지난 글](/posts/python-hello-world/)에서 첫 프로그램을 작성하고 실행했다. 코드를 쓰다 보면 곧 "이 코드가 왜 이렇게 되어 있지?"라는 의문이 생긴다. 6개월 후의 내가 또는 팀 동료가 코드를 이해하기 위해서는 주석이 필요하다. 그러나 주석을 많이 달면 좋은 것이 아니다. 잘못 쓴 주석은 코드를 더 이해하기 어렵게 만든다. 이번 글에서는 Python 주석의 종류와 올바른 작성법을 다룬다.
...
다음 글에서는 Python의 가장 독특한 특징 중 하나인 들여쓰기를 깊이 다룬다. 다른 언어에서는 선택이지만 Python에서는 문법인 들여쓰기, 올바른 규칙과 자주 만나는 오류를 살펴볼 것이다.
