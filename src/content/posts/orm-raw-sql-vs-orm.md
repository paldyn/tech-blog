...
[지난 글](/posts/olap-duckdb/)에서 DuckDB의 임베디드 OLAP 패턴을 살펴봤다. 이번 글부터는 ORM을 다룬다. 소프트웨어 개발에서 "Raw SQL을 써야 하는가, ORM을 써야 하는가"는 수십 년째 반복되는 논쟁이다. 정답 없는 선호 싸움처럼 보이지만, 실은 **상황에 따라 분명한 선택 기준**이 있다.
