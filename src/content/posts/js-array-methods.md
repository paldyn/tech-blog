...
[지난 글](/posts/js-async-patterns/)에서 모듈 시스템으로 코드 구조를 잡는 방법을 살펴봤습니다. 이번에는 실제 데이터를 다루는 일상적인 작업, 즉 **배열 조작**을 살펴봅니다. JavaScript의 배열 메서드는 `for` 루프를 대신해 선언적으로 데이터를 변환, 탐색, 집계하는 방법을 제공합니다. 어떤 메서드가 있고, 각각 언제 쓰는지를 정리합니다.
