...
[지난 글](/posts/js-async-await/)에서 Promise와 async/await의 기본 동작을 살펴봤습니다. 이번 글은 실전입니다. 실제 코드에서 비동기를 다루다 보면 "이걸 어떻게 구현하지?" 싶은 패턴들을 반복해서 마주칩니다. 여러 요청을 동시에 보내고 싶다, 실패하면 다시 시도하고 싶다, 오래 걸리면 포기하고 싶다 — 이런 패턴들을 체계적으로 정리합니다.
