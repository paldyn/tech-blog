...
[지난 글](/posts/ai-coding-review/)에서 AI 코드 리뷰 자동화 시스템을 살펴봤다. 이번 글부터는 AI 기술을 실제 서비스로 구현하는 **애플리케이션 구축 패턴**을 다룬다. 첫 번째 주제는 가장 대중적인 AI 응용 형태인 챗봇 서비스 설계다. 단순히 API를 호출하는 것을 넘어 대화 이력, 문서 검색, 안전 필터, 스트리밍 응답을 모두 갖춘 프로덕션 수준의 아키텍처를 살펴본다.
