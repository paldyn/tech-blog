...
[지난 글](/posts/java-migration-17-to-21/)에서 17→21 마이그레이션을 다뤘다. 세 번의 마이그레이션 글을 관통하며 반복적으로 등장한 주제가 하나 있다. 바로 **폐기(deprecated)되거나 제거(removed)된 API** 다. 이번 글에서는 이 주제를 한데 모아, 폐기가 어떤 단계를 거쳐 제거로 이어지는지, 그리고 레거시 코드에서 자주 마주치는 항목들을 무엇으로 대체할지 정리한다. 마이그레이션 작업에 곁에 두고 볼 실전 참고서 격의 글이다.
