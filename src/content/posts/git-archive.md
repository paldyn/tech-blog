...
[지난 글](/posts/git-diff-stat/)에서 `--stat`으로 변경 규모를 요약해 보는 법을 다뤘다. 이번엔 저장소를 다루는 또 다른 실무 작업, **소스 내보내기**다. 특정 버전의 소스 코드를 누군가에게 전달하거나 서버에 배포해야 할 때, `git clone`을 쓰면 `.git` 폴더에 담긴 전체 히스토리까지 통째로 따라온다. 받는 쪽은 커밋 기록이 필요 없는데도 무거운 `.git`을 떠안게 된다. <strong>`git archive`</strong>는 지정한 시점의 **파일 스냅샷만** tar나 zip으로 깔끔하게 묶어 내보낸다.
