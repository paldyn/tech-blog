...
[지난 글](/posts/ts-async-iterators/)에서 비동기 값의 연속을 다루는 이터레이터를 봤다. 비동기 작업을 시작하는 법은 충분히 다뤘으니, 이번에는 그 반대 — **시작한 작업을 취소하는 법**을 타이핑한다. 표준 도구는 `AbortController`다. fetch 요청 취소, 이벤트 리스너 정리, 타임아웃까지 하나의 인터페이스로 묶는데, TypeScript의 DOM·Node 타입에 이미 잘 정의되어 있어 타이핑은 비교적 매끄럽다. 다만 취소 에러를 구분하는 부분에 우리가 그동안 다룬 좁히기가 다시 등장한다.
